...
for (const id of ['boniu', 'bolo', 'mimo', 'dogegg']) {
  ...
  await writeFile(`${dir}/package.json`, JSON.stringify({
    ...
    files: ['index.js', 'index.d.ts', 'character.json', 'NOTICE.md', ...new Set(Object.values(pack.assets).map(asset => asset.src.replace(/^\.\//, '').split('/')[0]))], sideEffects: false,
  }, null, 2) + '\n')
