...
Dogegg uses a 256 px flat-vector rig with seven 12-cel clips (84 total), exported
to a PNG atlas. The AI-assisted design reference stays in `artwork/dogegg/`.
...
Optional `presentation` roles map locomotion (`n/ne/e/se/s/sw/w/nw`) and `peek` to existing reaction names. These roles are validated against the pack. They do not make reaction names a fixed enum; custom packs may omit them or supply different sprite styles.
