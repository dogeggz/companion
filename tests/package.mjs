...
const packMetadata = JSON.parse(await readFile(path.join(root, 'characters/dogegg/package.json'), 'utf8'))
const packArchive = `companion-kit-character-dogegg-${packMetadata.version}.tgz`
...
for (const file of [archive.filename, packArchive]) await cp(path.join(results, file), path.join(consumer, 'vendor', file))
install(path.join(consumer, 'vendor', archive.filename), path.join(consumer, 'vendor', packArchive))
...
assert.equal(JSON.parse(await readFile(path.join(installedPack, 'package.json'), 'utf8')).version, packMetadata.version)
assert.ok((await readFile(path.join(installedPack, 'NOTICE.md'), 'utf8')).includes('Artwork provenance'))
assert.ok(dogegg.reactions.appear && dogegg.reactions.disappear, 'Current portal animations are packaged')
